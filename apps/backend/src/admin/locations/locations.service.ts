...
export class LocationsService {
  ...
  async updateLocationRotator(locationId: string, configData: any) {
    return this.prisma.rotatorConfig.update({
      ...
      data: {
        ...
        offerSequence:
          typeof configData.offerSequence === 'string'
        ...
        weights:
          typeof configData.weights === 'string'
            ? configData.weights
            : JSON.stringify(configData.weights || {}),
        scarcityLimits:
          typeof configData.scarcityLimits === 'string'
  ...
  async resetPointer(locationId: string) {
    const config = await this.prisma.rotatorConfig.findUnique({
      where: { locationId },
    });
