...
export class AgentPortfolioService {
  ...
  async getPortfolio(agentId: string) {
    ...
    scanCounts.forEach((s: any) => {
      if (s.offerId) scanMap[s.offerId] = s._count.offerId;
    });
    claimCounts.forEach((c: any) => {
      if (c.offerId) claimMap[c.offerId] = c._count.offerId;
    });

    const now = new Date();

    const portfolio = businesses.map((biz: any) => {
      const bizOffers = allOffers.filter(
        (o: any) => o.businessName === biz.name,
      );
      const activeOffers = bizOffers.filter(
        (o: any) => o.status === 'approved' && o.endDate > now,
      );
      const totalScans = bizOffers.reduce(
        (sum: number, o: any) => sum + (scanMap[o.id] || 0),
        0,
      );
      const totalClaims = bizOffers.reduce(
        (sum: number, o: any) => sum + (claimMap[o.id] || 0),
        0,
      );
      const conversionRate =
        totalScans > 0 ? ((totalClaims / totalScans) * 100).toFixed(1) : '0.0';
  ...
  async getBusinessDetail(id: string, agentId: string) {
    ...
    const totalClaims = offers.reduce(
      (sum: number, o: any) => sum + o.claims,
      0,
    );
    const conversionRate =
      totalScans > 0 ? ((totalClaims / totalScans) * 100).toFixed(1) : '0.0';
